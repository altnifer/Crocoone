...
void drawMenu(char ** param, uint16_t str_count, uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1,
	...
	//fillRect(x0, y0, x1 - x0, y1 - y0, MAGENTA);//bg_color);
...
void drawIcon(const char* header_t, bool highlight,
	...
	if (highlight) {
		drawRect(x0, y0, x1 - x0, y1 - y0, txt_color);
		drawRect(x0 + 1, y0 + 1, x1 - x0 - 2, y1 - y0 - 2, txt_color);
		fillRect(x0 + 2, y0 + 2, x1 - x0 - 2, font.height + 3, txt_color);
...
void drawHeader(const char* header_t, FontDef font, uint16_t txt_color, uint16_t bg_color) {
	const uint8_t max_ch_in_str = ST7735_HEIGHT / font.width;
	char header[max_ch_in_str + 1];
	memset(header, 0, (max_ch_in_str + 1)*sizeof(char));
	strncpy(header, header_t, max_ch_in_str);
	uint16_t len = strlen(header);

	fillRect(0, 0, ST7735_HEIGHT, font.height + 4, bg_color);
	ST7735_WriteString((ST7735_HEIGHT - len * font.width) / 2, 2, header, font, txt_color, bg_color);
}

void drawErrorIcon(int error) {
	if (error == 1) {
	    fillScreen(BLACK);
	    fillTriangle(5, 36, 23, 5, 41, 36, YELLOW);
		ST7735_WriteString(50, 12, "WARNING", Font_11x18, YELLOW, BLACK);
		ST7735_WriteString(19, 17, "!", Font_11x18, BLACK, YELLOW);
		drawRect(5, 46, 150, 82, YELLOW);
		//ST7735_WriteString(11, 52, "please restart the", Font_7x10, YELLOW, BLACK);
		//ST7735_WriteString(11, 62, "device.", Font_7x10, YELLOW, BLACK);
	} else if (error == 2) {
	    fillScreen(BLACK);
	    fillTriangle(5, 36, 23, 5, 41, 36, RED);
		ST7735_WriteString(50, 12, "ERROR", Font_11x18, RED, BLACK);
		ST7735_WriteString(19, 17, "!", Font_11x18, BLACK, RED);
		drawRect(5, 46, 150, 82, RED);
		ST7735_WriteString(11, 52, "please restart the", Font_7x10, RED, BLACK);
		ST7735_WriteString(11, 62, "device.", Font_7x10, RED, BLACK);
	}
