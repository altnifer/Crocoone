...
{
	HAL_GPIO_WritePin(CSN_PORT, CSN_PIN, level);
	delay_us(5);
}
...
uint8_t read_register(uint8_t reg)
{
	...
	HAL_SPI_TransmitReceive(&NRF_SPI_PORT, (uint8_t*)0xff, &dt, 1, 1000);
	csn(HIGH);
	return dt;
}
...
uint8_t write_registerMy(uint8_t reg, const uint8_t* buf, uint8_t len)
{
	...
	csn(LOW);
	HAL_SPI_TransmitReceive(&NRF_SPI_PORT, &addr, &status, 1, 100);
	HAL_SPI_Transmit(&NRF_SPI_PORT, (uint8_t*)buf, len, 100);
	csn(HIGH);
	return status;
}
...
uint8_t write_register(uint8_t reg, uint8_t value)
{
	...
	csn(LOW);
	HAL_SPI_TransmitReceive(&NRF_SPI_PORT, &addr, &status, 1, 100);
	HAL_SPI_Transmit(&NRF_SPI_PORT, &value, 1, 1000);
	csn(HIGH);
	return status;
}
...
uint8_t write_payload(const void* buf, uint8_t data_len, const uint8_t writeType)
{
	...
	csn(LOW);
	HAL_SPI_TransmitReceive(&NRF_SPI_PORT, &addr, &status, 1, 100);
	HAL_SPI_Transmit(&NRF_SPI_PORT, (uint8_t*)current, data_len, 100);

	while(blank_len--)
	{
		uint8_t empt = 0;
		HAL_SPI_Transmit(&NRF_SPI_PORT, &empt, 1, 100);
	}
...
uint8_t read_payload(void* buf, uint8_t data_len)
{
	...
	csn(LOW);
	HAL_SPI_Transmit(&NRF_SPI_PORT, &addr, 1, 100);
	HAL_SPI_Receive(&NRF_SPI_PORT, (uint8_t*)current, data_len, 100);

	while(blank_len--)
	{
		uint8_t empt = 0;
		HAL_SPI_Receive(&NRF_SPI_PORT, &empt, 1, 100);
	}
...
uint8_t spiTrans(uint8_t cmd)
{
	uint8_t status = 0;
	csn(LOW);
	HAL_SPI_TransmitReceive(&NRF_SPI_PORT, &cmd, &status, 1, 1000);
	csn(HIGH);
	return status;
}
...
uint8_t getDynamicPayloadSize(void)
{
	...
	csn(LOW);
	addr = R_RX_PL_WID;
	HAL_SPI_TransmitReceive(&NRF_SPI_PORT, &addr, &result, 1, 1000);
	HAL_SPI_TransmitReceive(&NRF_SPI_PORT, (uint8_t*)0xff, &result, 1, 1000);
	csn(HIGH);
...
void toggle_features(void)
{
	...
	HAL_SPI_Transmit(&NRF_SPI_PORT, (uint8_t*)0x73, 1, 1000);
	csn(HIGH);
}
...
void writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
	...
	csn(LOW);
	HAL_SPI_Transmit(&NRF_SPI_PORT, &addr, 1, 1000);
	HAL_SPI_Transmit(&NRF_SPI_PORT, (uint8_t*)current, data_len, 1000);
	csn(HIGH);
}
...
///NEW///
void writeText(const void* buf, uint8_t len)
{
	const uint8_t* current = (const uint8_t*)buf;
	uint8_t data_len = rf24_min(len, 32);
	csn(LOW);
	HAL_SPI_Transmit(&NRF_SPI_PORT, (uint8_t*)0xA0, 1, 1000);
	HAL_SPI_Transmit(&NRF_SPI_PORT, (uint8_t*)current, data_len, 1000);
	csn(HIGH);
}
...
bool writeFast( const void* buf, uint8_t len/*, const bool multicast*/ )
{
	...
	while( ( get_status()  & ( 1 << TX_FULL ))) {			  //Blocking only if FIFO is full. This will loop and block until TX is successful or fail

		if( get_status() & (1 << MAX_RT)){
			...
			return 0;										  //Return 0. The previous payload has been retransmitted
		     ...
		     //Start Writing
	startFastWrite(buf, len, 0/*multicast*/, 1);
