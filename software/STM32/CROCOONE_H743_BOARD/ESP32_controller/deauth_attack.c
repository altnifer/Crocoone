...
void deauth_attack_main_task(void *main_task_handle);
...
void ESP32_start_deauth_attack(TaskHandle_t *main_task_handle) {
	xTaskCreate(deauth_attack_main_task, "deauth_attack", 1024 * 4, main_task_handle, osPriorityNormal1, NULL);
}
...
void deauth_attack_main_task(void *main_task_handle) {
	...
	//
		...
		if (!running_page && button_confirm_flag) {
			if (!send_cmd_with_check((cmd_data_t){DEAUTH_CMD, {get_target(), timeout, 0, 0, 0}}, error_buff, 2000))
				break;
			if (timeout) {
